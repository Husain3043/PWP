...
public class TrueTime {
    ...
    private static final com.example.user.plaspcik.DiskCacheClient DISK_CACHE_CLIENT = new  com.example.user.plaspcik.DiskCacheClient();
    private static final  com.example.user.plaspcik.SntpClient SNTP_CLIENT = new  com.example.user.plaspcik.SntpClient();
    ...
    public synchronized TrueTime withSharedPreferencesCache(Context context) {
        DISK_CACHE_CLIENT.enableCacheInterface(new  com.example.user.plaspcik.SharedPreferenceCacheImpl(context));
        return INSTANCE;
    }
     ...
     */
    public synchronized TrueTime withCustomizedCache( com.example.user.plaspcik.CacheInterface cacheInterface) {
    ...
    public synchronized TrueTime withRootDelayMax(float rootDelayMax) {
        if (rootDelayMax > _rootDelayMax) {
          String log = String.format(Locale.getDefault(),
            ...
            com.example.user.plaspcik.TrueLog.w(TAG, log);
        }
    ...
    public synchronized TrueTime withLoggingEnabled(boolean isLoggingEnabled) {
        com.example.user.plaspcik.TrueLog.setLoggingEnabled(isLoggingEnabled);
        return INSTANCE;
    }
    ...
    protected void initialize(String ntpHost) throws IOException {
        if (isInitialized()) {
            com.example.user.plaspcik.TrueLog.i(TAG, "---- TrueTime already initialized from previous boot/init");
            return;
        }
    ...
    synchronized static void saveTrueTimeInfoToDisk() {
        if (!SNTP_CLIENT.wasInitialized()) {
            com.example.user.plaspcik.TrueLog.i(TAG, "---- SNTP client not available. not caching TrueTime info in disk");
            return;
        }
